  ...
  return (
    <div className="absolute inset-0 pointer-events-auto select-none z-15">
      ...
      >
        ...
        {/*    General Purpose: Live CPU Clock, Temp & Load Mon     */}
        {/* ======================================================== */}
        <g
...
Status: ${isLiveConnected ? 'LIVE FEDORA KERNEL LINK' : 'SIMULATED'} // UPTIME: ${stats.os.uptime}`}
        >
          ...
          {/* Test Points & Ground Stitching Vias */}
          {[
          ...
          {/* Laser-Etched Real Fedora CPU Telemetry */}
          <g fill="#cfc6b4" fontFamily="Rajdhani, sans-serif" fontWeight="700" letterSpacing="0.8" opacity="0.9">
            <text x="56" y="22" fontSize="5.4" textAnchor="middle">{stats.cpu.model.slice(0, 20).toUpperCase()}</text>
            <text x="56" y="29" fontSize="4.0" fill="#8aa290" textAnchor="middle" letterSpacing="1.2">
              FEDORA 44 // {stats.cpu.cores}C/{stats.cpu.threads}T // KERNEL {stats.os.kernel.split('-')[0]}
            </text>
            <text x="56" y="88" fontSize="5.2" fill={theme.accent} textAnchor="middle">
              {stats.cpu.clockGhz.toFixed(2)} GHz // {stats.cpu.usage.toFixed(0)}% LOAD
            </text>
            <text x="56" y="94" fontSize="4.0" fill="#68767e" textAnchor="middle">
        ...
        {/*    General Purpose: Master Volume, Mute & Mic Control    */}
        {/* ======================================================== */}
        <g
...
(Flick toggle switches to Mute/Unmute Audio & Mic; Turn dial to adjust Volume)`}
        >
          ...
          {/* Real Vernier Master Volume Rotary Dial */}
          <g
          ...
          >
            ...
            {[...Array(24)].map((_, i) => {
              ...
              return (
                <line
                  ...
                  stroke={isVolLit ? theme.primary : '#506070'}
            ...
            {/* Pointer Rotates with Real Volume */}
            <g transform={`rotate(${(stats.audio.volume / 100) * 260 - 130})`}>
              <line x1="0" y1="-3" x2="0" y2="-10.5" stroke="#ffffff" strokeWidth="2.0" strokeLinecap="round" filter="url(#neon-phosphor-hard)" />
            </g>
            <circle cx="0" cy="0" r="2.8" fill="#ffd778" />
          </g>

          {/* TWO REAL SYSTEM TOGGLE SWITCHES (AUDIO MUTE & MIC MUTE) */}
          <g transform="translate(85, 26)">
            {/* Switch 1: AUDIO SINK MUTE / UNMUTE */}
            <g
              className="cursor-pointer"
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              ...
              <text x="16" y="44" fill={!stats.audio.sinkMuted ? theme.primary : '#ff4d4d'} fontSize="3.8" fontFamily="Share Tech Mono, monospace" textAnchor="middle">
              ...
              {/* Bat physically tilts based on real mute state */}
              <g transform={!stats.audio.sinkMuted ? 'translate(16, 24) rotate(-22)' : 'translate(16, 24) rotate(22)'}>
              ...
              <circle cx="16" cy="35" r="1.8" fill={!stats.audio.sinkMuted ? '#4de0ff' : '#ff4444'} filter="url(#neon-phosphor-hard)" />
            </g>

            {/* Switch 2: MICROPHONE MUTE / UNMUTE */}
            <g
            ...
            >
              ...
              <text x="16" y="44" fill={!stats.audio.sourceMuted ? '#88ff38' : '#ffaa33'} fontSize="3.8" fontFamily="Share Tech Mono, monospace" textAnchor="middle">
              ...
              <g transform={!stats.audio.sourceMuted ? 'translate(16, 24) rotate(-22)' : 'translate(16, 24) rotate(22)'}>
                <rect x="-2.2" y="-14" width="4.4" height="14" rx="2.0" fill="url(#ultra-chrome-bat)" stroke="#0c0e12" strokeWidth="0.6" />
                <circle cx="0" cy="-14" r="3.2" fill="url(#ultra-chrome-bat)" stroke="#0c0e12" strokeWidth="0.6" />
                <line x1="-0.8" y1="-13" x2="-0.8" y2="-2" stroke="#ffffff" strokeWidth="0.8" opacity="0.9" />
              </g>
              ...
              <circle cx="16" cy="35" r="2.8" fill="#1c2024" stroke="#4a5662" strokeWidth="0.6" />
              <circle cx="16" cy="35" r="1.8" fill={!stats.audio.sourceMuted ? theme.primary : '#ffaa33'} filter="url(#neon-phosphor-hard)" />
            </g>
          </g>
          ...
          {/* TEKTRONIX-STYLE RECESSED CRT OSCILLOSCOPE */}
          <g transform="translate(16, 78)">
            ...
            {/* Real Audio Waveform (Amplitude scales with Volume) */}
            <path
              d={`M 6 29 Q 24 ${29 - (stats.audio.volume / 100) * 18} 42 29 T 78 29 T 114 29 T 142 29`}
              fill="none"
              stroke={!stats.audio.sinkMuted ? theme.primary : '#687884'}
              strokeWidth="2.2"
              strokeLinecap="round"
              filter="url(#neon-phosphor-hard)"
            />
          ...
          {/* VINTAGE LED 7-SEGMENT MASTER VOLUME DISPLAY & STATUS ANNUNCIATORS */}
          <g transform="translate(172, 78)">
            ...
            <text x="35" y="18" fill={!stats.audio.sinkMuted ? theme.primary : '#ff5555'} fontSize="10.0" fontFamily="Share Tech Mono, monospace" fontWeight="bold" textAnchor="middle" filter="url(#neon-phosphor-hard)">
              {stats.audio.sinkMuted ? 'MUTED' : `${stats.audio.volume}% VOL`}
            </text>

            <g transform="translate(6, 38)">
              {/* Lamp 1: SINK ACTIVE */}
              <circle cx="0" cy="0" r="4.2" fill="#303842" stroke="#0e1216" strokeWidth="0.8" />
              <circle cx="0" cy="0" r="2.6" fill={!stats.audio.sinkMuted ? theme.accent : '#55606c'} filter={!stats.audio.sinkMuted ? 'url(#neon-phosphor-hard)' : undefined} />
              <circle cx="0" cy="0" r="0.8" fill="#ffffff" />
              <text x="0" y="9" fill="#7a8894" fontSize="3.2" fontFamily="Share Tech Mono, monospace" textAnchor="middle">AUDIO</text>

              {/* Lamp 2: KERNEL SYNC */}
              <circle cx="27" cy="0" r="4.2" fill="#303842" stroke="#0e1216" strokeWidth="0.8" />
              <circle cx="27" cy="0" r="2.6" fill={isLiveConnected ? theme.primary : '#885522'} filter="url(#neon-phosphor-hard)" />
              ...
              <circle cx="54" cy="0" r="2.6" fill={!stats.audio.sourceMuted ? '#4de0ff' : '#ff4d4d'} filter="url(#neon-phosphor-hard)" />
              <circle cx="54" cy="0" r="0.8" fill="#ffffff" />
              <text x="54" y="9" fill="#7a8894" fontSize="3.2" fontFamily="Share Tech Mono, monospace" textAnchor="middle">MIC</text>
            </g>
          </g>
        ...
        {/*    General Purpose: RAM Bargraph & NVMe/GPU Thermal Loop */}
        {/* ======================================================== */}
...
(Click to inspect memory footprint)`}
        >
          ...
          <g transform="translate(14, 18)">
            ...
            <text x="10" y="26" fill="#7a9684" fontSize="3.6" fontFamily="Share Tech Mono, monospace" textAnchor="middle">RAM</text>
            ...
            <text x="34" y="26" fill="#7a9684" fontSize="3.6" fontFamily="Share Tech Mono, monospace" textAnchor="middle">SWAP</text>
          </g>
          ...
          <g transform="translate(64, 12)">
            ...
            {/* Lit count dynamically proportional to RAM percent */}
            {(() => {
                ...
                return (
                  <g key={`dip-bar-${i}`} transform={`translate(${4 + i * 4.8}, 3)`}>
                    ...
                    <rect
                      ...
                      height="21.2"
                      rx="0.4"
                      fill={!isLit ? '#18241a' : isWarning ? '#ffb738' : theme.primary}
                      opacity={!isLit ? 0.25 : 0.95}
                      filter={isLit ? 'url(#neon-phosphor-hard)' : undefined}
                    />
                    <circle cx="1.7" cy="11" r="0.4" fill="#ffffff" opacity={isLit ? 0.9 : 0.15} />
                  </g>
                );
              });
            })()}
          ...
          <g fill="#7a9282" fontFamily="Share Tech Mono, monospace" fontSize="4.2">
            <text x="66" y="48">RAM: {stats.ram.usedMb}MB / {stats.ram.totalMb}MB ({stats.ram.percent}%)</text>
            <text x="146" y="48">{isLiveConnected ? 'LIVE' : 'MEM'}</text>
          </g>
...
(Click to probe hardware thermals)`}
        >
          ...
          {/* Analog Bourdon Tube Gauge Showing Real Disk / GPU Utilization */}
          <g transform="translate(262, 38)">
            ...
            <text x="0" y="-4" fill="#1a2028" fontSize="3.0" fontFamily="Rajdhani, sans-serif" fontWeight="700" textAnchor="middle">DISK%</text>
            <text x="0" y="8" fill="#505a66" fontSize="2.8" fontFamily="Share Tech Mono, monospace" textAnchor="middle">{stats.disk.percent}%</text>

            {/* Needle Points Directly to Disk Utilization Percentage */}
          ...
          <text x="32" y="60" fill="#8898a8" fontSize="4.4" fontFamily="Share Tech Mono, monospace">
            NVME: {stats.disk.tempC}°C // RTX 3050: {stats.gpu.tempC}°C (LOAD: {stats.gpu.utilization}%)
          </text>
        </g>
        ...
        {/*    General Purpose: VFD Matrix, Wire-Mesh GPU & Controls */}
        {/* ======================================================== */}
        <g
...
(Click Phoenix screw pins below to adjust Brightness or Lock Screen!)`}
        >
          ...
          <g transform="translate(16, 14)">
            ...
            <text x="28" y="49" fill="#7a8c9e" fontSize="3.6" fontFamily="Share Tech Mono, monospace">RTX-3050</text>
            <text x="28" y="55" fill="#586c7c" fontSize="3.4" fontFamily="Share Tech Mono, monospace">{stats.gpu.tempC}°C // {stats.gpu.memUsedMb}M</text>
          </g>
          ...
          {/* ZONE C: VACUUM FLUORESCENT DISPLAY (VFD) WITH REAL LINUX SPECS */}
          <g transform="translate(146, 14)">
            ...
            {[
              ...
              <g key={`vfd-row-${r}`} transform={`translate(4, ${4 + r * 13})`}>
                <text x="0" y="9" fill="#68e4a4" fontSize="6.0" fontFamily="Rajdhani, sans-serif" fontWeight="700" filter="url(#neon-phosphor-hard)">
                  {row.label}
                </text>
                <text x="24" y="9" fill="#d2ffea" fontSize="6.6" fontFamily="Share Tech Mono, monospace" fontWeight="bold" filter="url(#neon-phosphor-hard)">
                  {row.val}
                </text>
          ...
          {/* INTERACTIVE PHOENIX CONTACT SCREW TERMINAL BLOCK (FEDORA CONTROLS) */}
          <g transform="translate(24, 76)">
            ...
            {[
              ...
              >
                ...
                <text x="8" y="1" fill="#c0f0c0" fontSize="3.0" fontFamily="Share Tech Mono, monospace" textAnchor="middle">{term.pin}</text>
              </g>
            ))}
          </g>
        </g>
        ...
        {/*    General Purpose: Vernier Caliper Tracks Load Average  */}
        {/* ======================================================== */}
        <g
...
Vernier Caliper position dynamically tracks 1-minute system load.`}
        >
          ...
          {/* Machined Brass Vernier Caliper Slider (Tracks 1-Min Load Average) */}
          <g transform={`translate(${Math.max(60, Math.min(590, 100 + stats.loadAvg[0] * 35))}, 0)`}>
            <rect x="-8" y="-2" width="22" height="34" rx="2.5" fill="url(#ultra-brass-knurl)" stroke="#1a1202" strokeWidth="1.0" />
            <line x1="3" y1="0" x2="3" y2="30" stroke="#ffffff" strokeWidth="1.0" />
            <circle cx="3" cy="6" r="2.0" fill="#2a1e06" />
            <text x="3" y="24" fill="#181204" fontSize="4.8" fontFamily="Share Tech Mono, monospace" fontWeight="bold" textAnchor="middle">
