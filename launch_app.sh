...
if which brave-browser >/dev/null 2>&1; then
    exec switcherooctl launch brave-browser --user-data-dir="$PROFILE_DIR" --app="http://localhost:3000/" $GPU_FLAGS "$@"
elif which google-chrome >/dev/null 2>&1; then
    exec switcherooctl launch google-chrome --user-data-dir="$PROFILE_DIR" --app="http://localhost:3000/" $GPU_FLAGS "$@"
elif which chromium >/dev/null 2>&1; then
    exec switcherooctl launch chromium --user-data-dir="$PROFILE_DIR" --app="http://localhost:3000/" $GPU_FLAGS "$@"
else
